...
describe("EmailService", () => {
  ...
  describe("Email Sending", () => {
    ...
    test("should retry on failure and eventually succeed", async () => {
      ...
      let attemptCount = 0;
      const originalSend = emailService.providers[0].sendEmail.bind(
  ...
  describe("Rate Limiting", () => {
    test("should respect rate limits", async () => {
      ...
      const result1 = await limitedService.sendEmail({
        ...validEmail,
        id: "test-1",
      });
      expect(result1.success).toBe(true);
      ...
      const result2 = await limitedService.sendEmail({
        ...validEmail,
        id: "test-2",
      });
  ...
  describe("Circuit Breaker Integration", () => {
    test("should open circuit breaker after threshold failures", async () => {
      ...
      const health = emailService.getServiceHealth();
      const circuitBreakerStates = health.providers.map(
        (p) => p.circuitBreakerState
      );
